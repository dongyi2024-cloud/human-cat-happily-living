...
def compute_square_grid_by_long_side(
    real_width_m: float,
    real_height_m: float,
    long_side_cells: int = 60,
) -> tuple[int, int, float]:
    """根据真实空间尺寸生成正方形 cell 网格。"""
    if real_width_m <= 0 or real_height_m <= 0:
        raise ValueError("real_width_m 和 real_height_m 必须为正数")
    if long_side_cells <= 0:
        raise ValueError("long_side_cells 必须大于 0")

    long_side_m = max(real_width_m, real_height_m)
    cell_size_m = long_side_m / float(long_side_cells)
    grid_width = max(1, math.ceil(real_width_m / cell_size_m))
    grid_height = max(1, math.ceil(real_height_m / cell_size_m))
    return grid_height, grid_width, cell_size_m


def load_floor_plan_geometry(
    floor_plan_path: str,
    house_width_m: float = 16.5,
    house_depth_m: float = 16.5,
) -> dict[str, float | int]:
    """解析户型图，返回分析网格所需的真实尺寸和像素尺寸。"""
    from simulation_v9 import FloorPlanParser

    parser = FloorPlanParser(
        floor_plan_path,
        house_width_m=house_width_m,
        house_depth_m=house_depth_m,
    )
    parser.parse()
    return {
        "house_width_m": float(parser.house_width_m),
        "house_depth_m": float(parser.house_depth_m),
        "source_width_px": int(parser.img_width),
        "source_height_px": int(parser.img_height),
    }


def build_analyzer_from_floor_plan(
    floor_plan_path: str,
    **kwargs,
) -> "TrajectoryAnalyzer":
    """从户型图读取几何信息，构建与图片尺度一致的 analyzer。"""
    house_width_m = float(kwargs.pop("house_width_m", 16.5))
    house_depth_m = float(kwargs.pop("house_depth_m", 16.5))
    geometry = load_floor_plan_geometry(
        floor_plan_path,
        house_width_m=house_width_m,
        house_depth_m=house_depth_m,
    )
    return TrajectoryAnalyzer(**geometry, **kwargs)
...
class TrajectoryAnalyzer:
    ...
    def __init__(
        ...
        if grid_size is None:
            self.grid_height, self.grid_width, self.cell_size_m = compute_square_grid_by_long_side(
                self.house_width_m,
                self.house_depth_m,
                long_side_cells=long_side_cells,
            )
            self.cell_width_m = self.cell_size_m
            self.cell_height_m = self.cell_size_m
        else:
            ...
            self.cell_size_m = (self.cell_width_m + self.cell_height_m) / 2.0

        self.grid_shape = (self.grid_height, self.grid_width)
        self.grid_size = max(self.grid_shape)
        self.avg_cell_size_m = self.cell_size_m

        if proximity_cells is not None:
            self.proximity_m = float(proximity_cells) * self.cell_size_m
        else:
            self.proximity_m = float(proximity_m)
        self.proximity_cells = max(1, int(round(self.proximity_m / self.cell_size_m)))
    ...
    def load_from_csv(self, csv_path: str) -> None:
        ...
        csv_path = str(resolve_project_path(csv_path))
    ...
    def _to_grid(self, x: float, y: float) -> tuple[int, int]:
        """将仿真像素坐标映射到物理尺度格栅索引。"""
        x_m = (float(x) / self.source_width_px) * self.house_width_m
        y_m = (float(y) / self.source_height_px) * self.house_depth_m
        gx = int(np.clip(np.floor(x_m / self.cell_size_m), 0, self.grid_width - 1))
        gy = int(np.clip(np.floor(y_m / self.cell_size_m), 0, self.grid_height - 1))
    ...
    def _grid_distance_m(self, a: tuple[int, int], b: tuple[int, int]) -> float:
        """返回两个格栅中心点之间的近似物理距离。"""
        dy = (a[0] - b[0]) * self.cell_size_m
        dx = (a[1] - b[1]) * self.cell_size_m
    ...
    def _build_grids(self) -> None:
        ...
        print(
            ...
            f"单元: {self.cell_size_m:.3f}m x {self.cell_size_m:.3f}m | "
...
if __name__ == "__main__":
    ...
    analyzer = build_analyzer_from_floor_plan(floor_plan)
