...
from visualization_stage_v14 import build_context, render_grid_stage


def main():
    parser = argparse.ArgumentParser(description="生成 v14 栅格级可视化诊断图")
    parser.add_argument("--output-dir", default=os.path.join("result", "grid_stage_v14"), help="栅格级图片输出目录")
    ...
    outputs = render_grid_stage(ctx, args.output_dir)
    print("[grid_diagnostic_stage_v14] 输出完成:")
    for path in outputs:
        print(f"  - {path}")
